...
本版本继续打磨 MaiBot 更新页、插件页 WebUI 嵌入体验和未来复古界面密度：MaiBot 本体更新现在能直接看到目标版本说明，插件市场改用 WebUI 页面并保留原生插件管理入口，首页聊聊卡片也会尽量同步 WebUI 最近对话。

### MaiBot 更新
- MaiBot 本体更新弹窗改为左右两列布局，左侧选择更新目标，右侧展示目标版本 Release 更新说明或 Dev 分支最近提交。
- 新增 MaiBot 更新说明检查接口，会根据当前本地版本和目标 tag 汇总区间内的 Release 说明；选择 Dev 版本时展示 `dev` 分支最近提交。
- “其他版本”现在只提供“历史版本”和“Dev版本”两种更新方式：历史版本使用 Tag 更新，Dev 版本固定更新 `dev` 分支最新内容。
- 历史版本列表会过滤 `Easy` 开头的 tag，并默认选中最新历史 tag；选择 Dev 版本时不再显示 Tag 下拉框。
- 更新目标直接显示版本号，不再显示 `Tag 1.x.x` 这类前缀；当前已经是目标版本时会提示并禁用重复更新。
- 获取版本信息的进度条移动到版本切换下方，并保持固定高度，减少加载完成前后卡片高度跳动。
- 更新按钮移到左列底部并改为上箭头图标，刷新说明和查看更新移动到右列底部；移除更新弹窗左下角取消按钮。

### 插件页
- 插件页默认使用 MaiBot WebUI 插件市场，隐藏启动器原生插件市场；原生插件管理保留，可在插件管理中通过 WebUI / 原生切换。
- 插件管理和插件市场切换改为无外边框的 Tab 风格，并会记住 WebUI / 原生插件管理模式。
- WebUI 插件管理和插件市场切换时会保留已加载页面，减少切换后重复验证登录状态或重新卡在连接页的情况。
- 插件页在 MaiBot Core 未启动或 WebUI 未就绪时显示“插件未连接”语境的等待提示，而不是普通 WebUI 启动提示。
- 从首页插件入口打开配置时，WebUI 模式会直达 WebUI 插件配置页；原生模式仍打开启动器原生配置。

### 聊聊与首页
- 首页“聊聊”卡片在 WebUI 聊聊模式下也会显示最近对话预览，不再退化成只显示 WebUI 入口的服务卡片。
- WebUI 页面会记录当前 WebUI 用户身份，首页聊聊预览使用独立 WebUI 会话读取最近消息，避免混用启动器原生浏览记录。
- 本地聊天桥支持按 `sessionId` / `userId` 分会话保存消息、连接状态和引用关系，减少不同聊聊入口之间的消息串扰。
- 未来复古模式下，NapCat 未启动状态会以标题后的红色斜体“未启动”显示，并与标题字号和高度对齐。
- 现代 UI 下，一键包信息和聊聊卡片标题恢复显示，卡片信息层级更一致。

### 窗口与外观
- 默认窗口尺寸调整为 `1180 x 760`，最小尺寸调整为 `1000 x 680`。
- 顶栏高度、窗口控制按钮和右侧主服务按钮略微压缩，品牌文字从 “MaiBot OneKey” 改为 “MaiBot OK”。
- 未来复古模式下界面密度会随紧凑 / 默认 / 舒适缩放，插件市场卡片间距、卡片高度和按钮高度同步变化。
- 未来复古模式整体文字颜色略微加深，并在纸纹层后增加亮度提升层，避免叠加纸纹后背景显得偏暗。
...
聊聊页默认直接使用 MaiBot WebUI 嵌入聊天页面，保留启动器原生聊聊界面的开关；MaiBot 更新弹窗会记住上次选择的目标版本
标题栏目录入口改为直达 MaiBot 根目录；同时修正开发分支下 WebUI 依赖更新策略，减少分支切换后 Dashboard 版本不匹配的问题。

### 聊聊页面
- 聊聊页默认使用 MaiBot WebUI 的嵌入聊天页面，MaiBot Core 运行且 WebUI 就绪后可直接进入 WebUI 聊聊，不再始终停留在启动器原生聊天界面。
- 设置中心新增“聊聊页面”模式开关，可在 MaiBot WebUI 聊聊和启动器原生聊聊之间切换，并会保存到启动器用户配置。
- 悬浮球展开后的聊聊区域同步支持 WebUI 聊聊模式；未启用 WebUI 模式或 WebUI 尚未就绪时，会自动回退到启动器原生聊聊界面。
- 首页“聊聊”卡片会跟随当前模式和 WebUI 就绪状态切换展示：WebUI 可用时显示 WebUI 聊聊入口和 MaiBot Core 服务控制，未就绪时仍显示原生快捷聊天卡片。

### MaiBot 更新
- MaiBot 更新目标会记录到用户数据目录，包含选择的渠道、tag / 分支目标和选择时间。
- 再次打开 MaiBot 更新弹窗时，会优先恢复上次选择的目标；曾选择自定义 tag 或分支时，不会被默认正式版覆盖。
- 更新详情中新增“当前记录”，可直接看到启动器记录的 MaiBot 更新渠道和目标版本。
- MaiBot 更新弹窗新增远端版本获取进度条，会展示更新源读取、远端版本刷新、分支和 Tag 列表加载状态。
- 远端 MaiBot 版本获取失败时会显示具体更新源和错误信息，便于判断是网络、代理源还是 Git 访问异常。
- 修正 MaiBot 版本排序，`rc` / `pre` / `beta` 等预发布版本不再被误判为高于同号正式版，例如 `1.0.0` 会正确高于 `1.0.0-rc.4`。

### Python 依赖与 Dashboard
- Python 依赖检查现在会识别 MaiBot 仓库当前 Git 分支，并针对 `maibot-dashboard` 做分支化处理。
- MaiBot 位于 `main` 分支且声明了 `maibot-dashboard` 依赖时，会先移除本地 Dashboard 覆盖包，再进行依赖检查，避免旧覆盖包影响正式分支。
- MaiBot 位于非 `main` 分支且声明了 `maibot-dashboard` 依赖时，会强制将 Dashboard 标记为待更新，并允许安装预发布版本，便于开发分支使用最新 Dashboard。

### 界面与快捷入口
- 标题栏快捷按钮从“打开日志目录”调整为“打开 MaiBot 根目录”，图标改为文件夹，并保留 `Ctrl/Command + L` 快捷键。
- 当 MaiBot 根目录尚未就绪时，标题栏目录按钮会禁用，快捷操作会提示根目录未就绪，减少打开错误位置的情况。

### 打包
- 应用版本号更新到 `0.4.10`。
- 内置 SnowLuma 预置包更新到官方 v1.9.9 Windows x64 包。
- Windows 安装器会在解压后校验主程序和资源目录，失败时中断安装并写入 `install-failure.log`，避免只留下卸载程序的半成品目录。

## 0.4.9 - 2026-06-02

重点完善首次启动引导、未来复古界面、首页自定义内容、插件市场源配置、消息平台设置和适配器配置保存体验，同时修复 Python 覆盖依赖注入、WebUI 刷新导航和运行路径调试配置等问题。
...
- 进一步优化界面细节
- 为未来复古主题添加了夜间模式
- 首页支持自定义内容管理，可调整首页卡片内容、布局和展示区域。
- 标题栏支持直接切换深色模式
- 优化本地聊天未连接时的提示，未连接或等待 WebUI 聊天服务时状态更清楚。

### 插件市场与远程源
- 插件市场支持切换远程源，新增源配置管理能力，可在官方 GitHub 源和自定义源之间切换。
- 插件市场添加惊喜推荐。
- 插件商店支持类型筛选。
- 插件管理支持图标显示
...
### 其他
- 将首页webui端口配置移动到设置中心
- 修复网页浏览器刷新导航失败的问题，减少刷新后地址或页面状态异常。
- 不再允许自定义python路径。
- 优化模块更新、Python 依赖管理和服务管理相关流程。
- 更新内置Maibot到1.0.0-rc.4
...
- Windows 打包保留 NapCat 根目录的版本清单 `config.json`，同时继续排除数据库文件和日志文件，避免新安装缺少启动所需文件。

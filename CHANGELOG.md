...
- 首页和设置中的插件快捷入口改为直接在 MaiBot Dashboard 中打开对应页面。
- 移除内置 OpenCode CLI、启动入口、相关设置、IPC、随包说明资源与发布前校验要求。
- 移除首页中重复的“聊聊”和消息平台聚光浮层引导，统一使用侧边新手引导。
- 启动器主窗口默认尺寸由 1180 × 760 调整为 1100 × 720。
- 未来复古首页将快捷操作与麦麦卡片并排显示，并统一两张卡片的高度。
- 麦麦卡片下方提供小号编辑/保存与恢复默认布局按钮，进入编辑模式后齿轮会切换为保存图标。
- 移除首次启动协议确认标题旁的装饰图标。
...
- 显示 Maibot更新进度
- 优化插件页面流畅度
- 更新内置 Maibot 版本到 1.1.3

## 0.4.19 - 2026-07-28

- 插件管理和插件市场统一使用 MaiBot WebUI 页面，并在切换时保留已加载内容。
...
- 更新预置maibot版本到1.0.9
- 修复napcat无法修复的问题
- 重新安装 NapCat / SnowLuma 前会清理残留后端进程，文件仍被占用时给出明确提示。
- NapCat / SnowLuma 组件升级改为先复制到临时目录再切换，降低文件占用导致目录半损坏的风险。
- MaiBot Core 或 QQ 后端运行中禁止更新一键包，避免安装器替换正在使用的资源。
- 内置 SnowLuma 预置包更新到官方 v1.11.1 Windows x64 包。

## 0.4.14 - 2026-06-22

- 修复插件重复请求的问题
- 新增导入导出maibot数据功能（不含插件）
- 内置 SnowLuma 预置包更新到官方 v1.10.2 Windows x64 包。
...
### 插件配置编辑增强
- 插件配置弹窗新增“可视化 / TOML 源码”两种编辑模式。
- 关闭未保存配置时会弹确认。
- 新增“重置配置”，会先备份再恢复默认或删除本地 config.toml。
- 配置页新增“详情”页，展示插件 ID、版本、作者、路径、配置路径、链接等。
- 插件卡片更紧凑，桌面上改成更多列展示。
- 插件编写器入口从插件面板内部挪到顶部工具栏。
- 插件 MaiBot 版本兼容判断现在只比较主版本/次版本，避免补丁号导致过严判断。
...
### 插件页
- 插件页默认使用 MaiBot WebUI 插件市场。
- 插件管理可使用MaiBotWebUI内置界面。

### 首页
- 首页“聊聊”卡片在 WebUI 聊聊模式下也会显示最近对话预览。

### 窗口与外观
- 默认窗口尺寸调整为 `1180 x 760`，最小尺寸调整为 `1000 x 680`。
- 未来复古模式下界面密度配置更加明显。
- 未来复古模式整体文字颜色略微加深，略微提高背景亮度以提高可读性。


## 0.4.10 - 2026-06-07

### 聊聊页面
- 聊聊页默认使用 MaiBot WebUI 的嵌入聊天页面。
...
### 界面与快捷入口
- 标题栏快捷按钮从“打开日志目录”调整为“打开 MaiBot 根目录”。
